...
EAP-Core validation example #1. Exposes the `bankdw` payments data
warehouse (5 tables, ~3000 rows of seed data) as an MCP stdio server
backed by an in-memory DuckDB.

This is one of two MCP servers (alongside `sfcrm-mcp-server`) that the
...
on a realistic data-backed workload.

## What it does

On startup:

1. Loads every CSV under `$BANKDW_DATA_DIR` (default
   `../../samples/bankdw`) into a fresh in-memory DuckDB. Table name =
   CSV stem.
2. Parses `$BANKDW_SCHEMA_CSV` (default
   `../../samples/bankdw_schema.csv`) into typed per-column metadata
   (`ColumnInfo` / `TableSchema`).
3. Registers three tools on an `McpToolRegistry` and serves them over
   MCP stdio.

Tables loaded from the seed data: `bridge_party_account`, `dim_bank`,
`dim_party`, `dim_product`, `fact_payments`.
...
```bash
uv run --with mcp --with duckdb python examples/bankdw-mcp-server/server.py
```
  ...
  "mcpServers": {
    "bankdw": {
      ...
      "args": [
        ...
        "--with", "duckdb",
        "python",
        "/absolute/path/to/examples/bankdw-mcp-server/server.py"
      ]
    }
  }
}
...
Two test files - `tests_bankdw/test_tools.py` covers tool logic
(list/describe/query, JOIN, truncation, error paths);
`tests_bankdw/test_query_safety.py` covers the read-only guard's reject paths
...
CSVs live at `samples/bankdw/` at the workspace root and are not
shipped with the SDK package - they're committed alongside the
validation examples. The schema CSV at `samples/bankdw_schema.csv`
is the source of truth for column metadata.

## See also

- `examples/sfcrm-mcp-server/` - the second validation MCP server
  (Salesforce CRM data).
