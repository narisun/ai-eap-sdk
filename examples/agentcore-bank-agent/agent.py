"""bank-agent — reference implementation on AWS Bedrock AgentCore.

Mirrors `docs/user-guide-aws-agentcore.md` end-to-end. Boots locally
without AWS credentials (stubs swap in for cloud calls). Flip
`EAP_ENABLE_REAL_RUNTIMES=1` and configure boto3 credentials to
graduate to real AgentCore Memory / Code Interpreter / Browser /
Registry / Payments / Evaluations calls.
"""
...
def build_client() -> EnterpriseLLM:
    ...
    Swap `provider="local"` → `provider="bedrock"` when ready and set
...
async def execute_transfer(from_id: str, to_id: str, amount_cents: int) -> dict:
    ...
    2. If the downstream rate-quote service responds with HTTP 402,
       the PaymentBackend signs and the caller retries.
    """
    client = build_client()
    pay = build_payments()
    ...
    try:
        ...
        # Real flow: pay.authorize_and_retry(pr) then re-issue.
...
async def publish_agent_card() -> str:
    """Publish this agent's A2A card to the Registry."""
